...
export async function GET(request: NextRequest) {
    try {
        ...
        const rtoId = request.nextUrl.searchParams.get('rtoId');
        ...
        const buildQuery = (options: { includeDeletedFilter: boolean; includeRelations: boolean }) => {
            let query = adminSupabase
                ...
                .select(options.includeRelations
                    ? '*, rto:rtos(id, name, code), application:applications(id, application_number, student_first_name, student_last_name, student_uid)'
                    : '*');
        ...
        const bills = (data || []).filter((bill) => {
            const billWithDeleteFlag = bill as { is_deleted?: boolean | null };
            return billWithDeleteFlag.is_deleted !== true;
        });

        return NextResponse.json({
            bills: redactFinancialDataForRole(bills, authz.context.role),
        });
    } catch (error) {
        return NextResponse.json(
