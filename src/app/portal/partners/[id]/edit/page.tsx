 ...
 * Edit Partner Page
 * 
 * Form for editing an existing partner (agent, subagent, or provider)
 */
...
import type { PartnerType, PartnerStatus, PriorityLevel, ContactChannel, Partner } from '@/types/database';
...
export default function EditPartnerPage({ params }: Props) {
    const { id } = use(params);
    ...
    useEffect(() => {
        async function fetchPartner() {
            try {
                ...
                const partner = result.data as Partner;
                setFormData({
                    type: isPartnerType(partner.type) ? partner.type : 'agent',
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            router.push(`/portal/partners/${id}`);
        } catch (err) {
    ...
    const pageTitle = `Edit ${PARTNER_TYPE_LABELS[selectedType]}`;
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground">Update partner information</p>
                        </div>
                    </div>
                </div>
            </header>
