...
import { Loader2, Link2, Unlink, Mail, AlertCircle, CheckCircle2, Lock, Shield } from 'lucide-react';
    ...
    const handleLinkGoogle = async () => {
        ...
        try {
            const { error } = await supabase.auth.linkIdentity({
                provider: 'google',
                options: {
                    redirectTo: `${window.location.origin}/auth/callback?next=/portal/settings/account`,
                },
            });
    ...
    const handleUnlinkIdentity = async (identity: UserIdentity) => {
        ...
        try {
            ...
            if (error) {
                ...
                setMessage({ type: 'success', text: `${providers[identity.provider]?.name || identity.provider} account unlinked successfully` });
                await loadUser();
            }
        } catch {
            setMessage({ type: 'error', text: 'An unexpected error occurred' });
    ...
    const handleEmailChange = async (e: React.FormEvent) => {
        ...
        try {
            const { error } = await supabase.auth.updateUser({
                ...
                emailRedirectTo: `${window.location.origin}/auth/callback?next=/portal/settings/account`,
            });
            ...
            if (error) {
                ...
                setEmailMessage({
                    type: 'success',
                    text: 'A confirmation email has been sent to your new email address. Please click the link to verify the change.'
                });
                setNewEmail('');
            }
        } catch {
        ...
        <div className="p-8 max-w-2xl space-y-8">
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        Connect multiple login methods to your account. Even if your Google account uses a different email, you&apos;ll still access the same account.
                    </CardDescription>
                </CardHeader>
            ...
            <Card className="border-blue-200 bg-blue-50/50 dark:border-blue-800 dark:bg-blue-950/20">
                <CardHeader>
                    ...
                    <CardDescription>
                        You will automatically receive email notifications for security-sensitive actions:
                    </CardDescription>
                </CardHeader>
                <CardContent>
